...
func run() -> void:
	...
	check("fixed-jump-height", game.player.jumps == 1 and game.player.air_jumps_total == 0 and absf((320-min_y)-53.3333) < 5, {"rise_px":320-min_y, "jumps":game.player.jumps})
	await steps(30)
	check("held-jump-no-bounce", game.player.jumps == 1 and game.player.is_on_floor(), {"jumps":game.player.jumps})
	# Web-zip: one fresh press in the air adds a second jump; a third press does nothing.
	await fresh()
	game.player.test_jump_pressed = true
	min_y = game.player.position.y
	for i in range(70):
		await steps(1)
		min_y = minf(min_y, game.player.position.y)
		if i == 18 or i == 30:
			game.player.test_jump_pressed = true
	check("web-zip-double-jump-once", game.player.jumps == 1 and game.player.air_jumps_total == 1 and 320 - min_y > 90 and 320 - min_y < 2.2 * 53.34, {"rise_px":320-min_y, "jumps":game.player.jumps, "air_jumps":game.player.air_jumps_total})
	# Every skyscraper roof sits higher than one jump can reach from any neighbouring roof.
	var min_rise := 1000.0
	for entry in game.level.solids:
		if entry[3] < game.level.skyscraper_min_height:
			continue
		for other in game.level.solids:
			if other[3] < game.level.skyscraper_min_height and absf(other[0] + other[2] - entry[0]) < 80:
				min_rise = minf(min_rise, other[1] - entry[1])
	check("skyscrapers-need-web-zip", min_rise > 53.34 and min_rise < 2.0 * 53.34, {"smallest_rise_px":min_rise, "single_jump_px":53.33})
	...
	await steps(4)
	check("actual-taxi-collision", game.state == Game.State.DYING and game.deaths == 1 and "taxi" in game.death_reason, {"state":game.state,"deaths":game.deaths,"reason":game.death_reason})
	...
	await steps(1)
	check("fall-boundary", game.state == Game.State.DYING and "civilians" in game.death_detail, {"state":game.state,"detail":game.death_detail})
	await fresh()
	var starts: Array = game.movers.map(func(m): return m.area.position.x)
	await steps(40)
	var moved := true
	for i in range(game.movers.size()):
		var m: Dictionary = game.movers[i]
		var x: float = m.area.position.x
		moved = moved and not is_equal_approx(x, starts[i]) and x >= m.from - 0.01 and x <= m.to - 24 + 0.01
	check("moving-taxis-patrol", game.movers.size() >= 4 and moved, {"start":str(starts),"now":str(game.movers.map(func(m): return m.area.position.x))})
	game.restart_attempt()
	check("taxis-reset-on-retry", game.movers.all(func(m): return is_equal_approx(m.area.position.x, lerpf(m.from, m.to - 24, 0.5 - 0.5 * cos(TAU * m.phase)))), {"now":str(game.movers.map(func(m): return m.area.position.x))})
	await fresh()
	var route = Route.new()
	var route_ticks := 0
	while game.state == Game.State.PLAYING and route_ticks < 2500:
		...
		await steps(1)
		route_ticks += 1
	check("complete-real-route", game.state == Game.State.COMPLETE and game.deaths == 0 and route.next_air == route.air_marks.size(), {"state":game.state,"deaths":game.deaths,"ticks":route_ticks,"position":str(game.player.position),"jump_marks_used":route.next_jump,"web_zips_used":route.next_air})
	game.start_session()
	game.start_session()
